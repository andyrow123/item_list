...
RSpec.describe ItemPrinter do
  ...
  context '#to_summary' do
    ...
    before(:each) do
      ...
      allow(item).to receive(:keys).and_return(%w(name description price))
      allow(item).to receive(:map).and_return(['Test Dish 1          ', '10.95          '])
      allow(item).to receive(:join).and_return('Test Dish 1          10.95          ')
    end

    it { is_expected.to respond_to(:to_summary).with(3).arguments }

    it 'includes index' do
      expect(described_class.to_summary(item, ['name', 'price'], 0)).to match(/^0\./)
    end

    it 'includes name' do
      expect(described_class.to_summary(item, ['name', 'price'], 0)).to match(/Test Dish 1/)
    end

    it 'does not include description' do
      expect(described_class.to_summary(item, ['name', 'price'], 0)).not_to match(/Test dish 1 description/)
    end

    it 'includes price' do
      expect(described_class.to_summary(item, ['name', 'price'], 0)).to match(/10.95/)
    end
