...
RSpec.describe ItemListPrinter do
  ...
  context 'printing a summary of items' do
    ...
    it 'sends each item to the item_printer' do
      expect(item_printer_module)
        .to receive(:to_summary)
        .with(items[0], %i[name price], 1)
      expect(item_printer_module)
        .to receive(:to_summary)
        .with(items[1], %i[name price], 2)

      described_class.to_summary(items, %i[name price], item_printer_module)
    end

    it 'separates item_printer output onto separate lines' do
      items.each { |item| allow(item).to receive(:[]).and_return('a item') }

      allow(item_printer_module)
        .to receive(:to_summary).and_return('a item')

      expect(described_class.to_summary(items, %i[name price], item_printer_module))
        .to eq("a item\na item")
    end
  end
end
